...
- **Política de Privacidade Atualizada**: Atualizadas as páginas [privacidade.html](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/privacidade.html) e [public/privacidade.html](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/public/privacidade.html) com o escopo unificado da plataforma **Na Régua** e da Barbearia Vieira.
- **Adaptação Automática ao Tema do Sistema (Dark/Light)**: Configurado `userInterfaceStyle: "automatic"` no [app.json](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app.json) e sincronizado em [ThemeContext.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/contexts/ThemeContext.tsx) para que o aplicativo acompanhe em tempo real o modo claro/escuro configurado no smartphone do cliente.
- **Gráfico de Recursos (Feature Graphic) da Play Store (1024x500)**: Criado o asset oficial em `assets/play-store-banner-1024x500.png` e adicionado o script [scripts/formatar-banner.ps1](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/scripts/formatar-banner.ps1) para geração e formatação automática de banners promocionais.
- **Ajustes de Build de Produção (EAS / Gradle)**: Inclusão de `GRADLE_OPTS: "-Xmx4096m -XX:MaxMetaspaceSize=1024m"` em [eas.json](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/eas.json) e incremento do `versionCode` para 3 em [app.json](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app.json); bundle AAB de produção `barbearia-vieira-v3.aab` gerado e binários de build adicionados ao `.gitignore`.
- **Precisão de GPS e Coordenadas da Barbearia Vieira**: Correção do hook [hooks/useLocalizacao.ts](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/hooks/useLocalizacao.ts) e inclusão das migrations `20260904120000_update_barbearia_vieira_real_coordinates.sql` e [scripts/atualizar_coordenadas_vieira.sql](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/scripts/atualizar_coordenadas_vieira.sql) com a latitude/longitude exatas da unidade física para cálculo de rota e distância na vitrine.
- **Interligação Web e Conformidade**: Links diretos entre a landing page ([index.html](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/index.html)), a Política de Privacidade ([privacidade.html](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/privacidade.html)) e a Página de Exclusão de Conta ([excluir-conta.html](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/excluir-conta.html)) tanto na raiz quanto em `public/`; script [scripts/configurar_crons.sql](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/scripts/configurar_crons.sql) pré-configurado com o endpoint oficial do projeto Supabase.

## Gestão Inteligente de Serviços, Biblioteca Ilustrada, Equipe e Expediente (03/09/2026)

- **Modal Otimizado de Novo/Editar Serviço**: Em [mais.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/mais.tsx), reestruturado o modal com `modalEditorSheet` aproveitando o topo da tela (`marginTop: 36`, `flex: 1`), mantendo foto, nome, sugestão inteligente e campos essenciais visíveis confortavelmente acima do teclado virtual.
- **Dedução Inteligente de Ilustrações e Descrições de Serviços**: Em [IlustracaoServico.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/components/IlustracaoServico.tsx) e [descricoesServicos.ts](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/lib/descricoesServicos.ts), catálogo ampliado com suporte específico a serviços especializados (Infantil, Nevou, Platinado, Luzes Masculinas, Hot Towel, Moicano, Pigmentação, Acabamento, etc.), com sugestões inteligentes e ilustrações full-bleed de alta definição.
- **Intervalos Avançados e Expediente Completo**: Em [opcoes-avancadas.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/opcoes-avancadas.tsx), adicionada configuração de intervalos dinâmicos de atendimento, controle detalhado de expediente e percentual padrão de comissão.
- **Gerenciamento de Equipe e Filiais**: Criação da tela dedicada [gerenciar-equipe.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/gerenciar-equipe.tsx), comissionamento por profissional, associação de barbeiros a filiais e sincronização via [BarbeariaContext.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/contexts/BarbeariaContext.tsx) e [gestao-barbearia.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/gestao-barbearia.tsx).

## Galeria de Mídias Ampliada (30 slots), Landing Page Web e Mimo de Retenção (01/09/2026)

- **Galeria com Reordenação e Suporte a Vídeos**: Criação do componente [MidiaGaleriaCard.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/components/MidiaGaleriaCard.tsx), suporte para até 30 mídias no perfil da barbearia com reordenação interativa e migration `20260901140000_allow_videos_in_storage.sql`.
- **Landing Page Web Oficial (Vercel & Cloudflare)**: Desenvolvimento de página institucional em `index.html`, `styles.css`, `script.js` e espelho em `public/` para divulgação da plataforma Na Régua e download do app.
- **Regra de Mimo e Retenção de Clientes (40 dias)**: Migration `20260901110000_mimo_40_dias.sql` aprimorando a rotina de envio de mimos promocionais para clientes inativos há mais de 40 dias.

## Redesign Apple Style ("Meu Negócio") e Desempenho Operacional (29/08/2026)

- **Redesign Visual Padrão Apple nas Telas do Barbeiro**: Modernização visual com acabamento premium, cards espaçados e micro-interações em [hoje.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/hoje.tsx), [semana.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/semana.tsx) e [clientes.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/clientes.tsx).
- **Métricas e Desempenho Mensal**: Criação do hook [useDesempenhoMensal.ts](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/hooks/useDesempenhoMensal.ts) com relatórios consolidados de faturamento, novos clientes e taxa de conversão.

## Liberação de Todos os Dias (Segundas Inclusas), Mês Inteiro e Remoção de Fila Virtual (28/08/2026)

- **Todos os Dias da Semana (Segunda a Domingo)**: Removida a restrição legada `isSegunda` em `diasJanelaContinua` ([horario.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/agendamento/horario.tsx)) e atualizado [preparar-agenda.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/preparar-agenda.tsx) para 7 dias completos.
- **Pré-configuração de Liberação do Mês Inteiro (30 Dias)**: Criada a aba **"Mês Inteiro (30d)"** em `preparar-agenda.tsx` que gera as 4 semanas consecutivas e permite salvar o mês inteiro em lote via RPC `salvar_agenda_multiplas_semanas_rpc`.
- **Presets Rápidos de 1 Toque**: Adicionada barra de ações rápidas (`Seg a Sáb`, `Ter a Sáb`, `Todos (7 Dias)`, `Limpar`) para configurar os dias de trabalho e folgas instantaneamente.
- **Remoção do Modo Fila Virtual**: Removido o modo `fila_virtual` de [gestao-barbearia.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(barbeiro)/gestao-barbearia.tsx), [useBarbearias.ts](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/hooks/useBarbearias.ts), [index.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(tabs)/index.tsx) e [servicos/index.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(tabs)/servicos/index.tsx), mantendo os modos profissionais **Agenda Contínua** e **Abertura por Drops**.
- **Imagens Oficiais e Logos na Vitrine**: Em [app/(app)/barbearias/index.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/barbearias/index.tsx) e [app/(app)/barbearias/[slug].tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/barbearias/[slug].tsx), adicionado fallback direto para o banner e a logo oficiais da Barbearia Vieira (`assets/barbearia-vieira-banner.png` e `assets/barbearia-vieira-logo.png`), garantindo identidade visual completa e sem placeholders na vitrine.
- **Onboarding Direto na Vitrine para Clientes sem Barbearia**: Em [app/_layout.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/_layout.tsx), [app/(app)/(tabs)/index.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/app/(app)/(tabs)/index.tsx) e [contexts/BarbeariaContext.tsx](file:///c:/Users/Alex/Documents/WORKSPACE/MEUS%20APPS/App%20Barbearia/contexts/BarbeariaContext.tsx), qualquer cliente que nunca tenha escolhido uma barbearia é direcionado imediatamente para a Vitrine de Barbearias (`/(app)/barbearias`) para explorar os estabelecimentos e escolher o seu preferido.
- **Migration `20260828110000_fix_date_cast_in_agenda_rpcs.sql`**: Corrigido o cast explícito `v_dia.data::date` na inserção da tabela `dias_agenda` dentro de `salvar_agenda_semanal_rpc` e `salvar_agenda_multiplas_semanas_rpc`, eliminando o erro `column "data" is of type date but expression is of type text`.
- **Migration `20260828100000_enable_all_days_month_and_remove_fila_virtual.sql` e `20260828090000_fix_barbearia_membros_and_staff_rls.sql`**: Aplicadas com sucesso no banco remoto Supabase via `npx supabase db push --linked`.
- **Validação Local & Remota**: Schema cache sincronizado, RPCs `salvar_agenda_multiplas_semanas_rpc` e `salvar_agenda_semanal_rpc` ativas no banco remoto e `npx tsc --noEmit` com **0 erros**.

## Auditoria Completa de Agendamento, Vagas e Multi-tenant (28/08/2026)

- **Causa Raiz 1 (Carregamento de Barbeiros em `useAgendamento.ts`)**: Quando `barbearia_membros` retornava vazio (ou era bloqueado por RLS para clientes anônimos/clientes), o hook `useAgendamento` zerava a lista de barbeiros (`setBarbeiros([])`), impedindo que `barbeiroSelecionadoId` fosse definido na tela de horários. Sem o barbeiro selecionado, `buscar_slots_disponiveis` não era chamado e a tela mostrava sempre *"O profissional ainda não disponibilizou horários para esta data"*. Corrigido com fallback resiliente buscando barbeiros ativos na tabela `perfis`.
- **Causa Raiz 2 (Bloqueio de RLS ao Liberar/Salvar Vagas pelo Barbeiro)**: A função de segurança `usuario_e_barbeiro(p_barbearia_id)` dependia estritamente de `barbearia_membros`. Criada a migration `20260828090000_fix_barbearia_membros_and_staff_rls.sql` com RPCs atômicas `liberar_vagas_tarde_rpc` e `salvar_agenda_semanal_rpc` (`SECURITY DEFINER`) e sincronização de vínculos.
- **Causa Raiz 3 (Barbearia Nula em `BarbeariaContext.tsx`)**: Se um barbeiro entrasse sem `ultima_barbearia_id` no perfil, o contexto ficava nulo. Adicionado fallback para vincular automaticamente barbeiros/admins à barbearia ativa principal.
- **Causa Raiz 4 (Reserva Direta no `confirmacao.tsx`)**: Simplificado o fluxo para chamar `reservar_slot` diretamente com `params.slotId` sem depender de subconsultas em `slots_agenda` sujeitas a RLS no cliente.
- **Causa Raiz 5 (Hero Card da Tela Inicial no Modo Contínuo)**: `agendaPermiteReserva` em `index.tsx` agora permite reservas tanto em modo `continua` quanto em `drops` aberto, mostrando o card de abertura programada apenas quando o modo for `drops` e a agenda estiver programada.
- **Causa Raiz 6 (Acesso Direto a Serviços em `barbearias/[slug].tsx`)**: Serviços na vitrine da barbearia agora são clicáveis e direcionam diretamente para a escolha de horário com o serviço e a barbearia selecionados.
- **Validação Local**: `npx tsc --noEmit` executado com **0 erros**.
